...
.NOTES
    ...
    - AutoShutdown: "8-18" - Start/stop hours; no AutoShutdown tag means the VM is not managed
...
.EXAMPLE
    ...
    2. Review current status (shows the stop time from the AutoShutdown tag, e.g. "shut down at 18:00 today")
...
.FUNCTIONALITY
    Status Indicators:
    - ❌ Red: "VM will shut down at <stop> today" - Managed, no exclusions active
    - ✅ Green: "VM shutdown is DISABLED for today" - Temporary daily skip active
    - 🔒 Orange: "VM shutdown is DISABLED by administrator" - Skip-until / weekday exclusion, or VM not managed
...
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
    ...
    <Grid Margin="15">
        ...
        <!-- Info Text -->
        <TextBlock Text="Note: this VM's shutdown time comes from its AutoShutdown tag unless excluded."
...
function Update-StatusDisplay {
    try {
        ...
        # The AutoShutdown tag ("start-stop", e.g. 8-18) is what the runbook acts on.
        # No tag = the VM is not managed at all (this replaces the old permanent-exclude tag).
        $autoShutdown = $tags."AutoShutdown"
        $stopLabel = "its scheduled time"
        if ($autoShutdown) {
            $stopPart = ($autoShutdown -split "-")[1]
            if ($stopPart) { $stopLabel = "{0:D2}:00" -f [int]($stopPart.Trim()) }
        }
        ...
        if ($isExcluded) {
            ...
            elseif ($exclusionType -eq "ADMIN_SKIP" -or $exclusionType -eq "WEEKDAY") {
                $txtStatus.Text = "🔒 VM shutdown is DISABLED by administrator`n$exclusionReason"
                $txtStatus.Foreground = 'Orange'
            }
        } elseif (-not $autoShutdown) {
            $txtStatus.Text = "🔒 This VM is not managed by AutoShutdown`nNo AutoShutdown tag - the runbook ignores it."
            $txtStatus.Foreground = 'Orange'
        } else {
            $txtStatus.Text = "❌ VM will shut down at $stopLabel today`nNo exclusion tags active"
        ...
        $tagDisplay = ""
        if ($tags) {
            if ($tags."AutoShutdown") {
                $tagDisplay += "⏰ Schedule: $($tags.'AutoShutdown')`n"
            }
            if ($tags."AutoShutdown-SkipUntil") {
...
$btnClearToday.Add_Click({
    $confirmation = [System.Windows.MessageBox]::Show(
        "Are you sure you want to clear the skip tag for today? The VM will shut down at its scheduled time.",
