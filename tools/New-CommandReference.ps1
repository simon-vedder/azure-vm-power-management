...
.EXAMPLE
# Regenerate everything and publish the whole reference to the tools site.
...
$scriptPath = Join-Path $repoRoot 'src' 'runbooks' 'Invoke-AzureVMPowerManagementRunbook.ps1'
$documented = @($commands)
...
foreach ($command in $documented) {
    ...
    $sitePages[$fileKey] = [pscustomobject]@{
        ...
        GroupNote = if ($isScript) {
            ...
            'The commands the runbook calls, and the same ones you can run locally after Install-Module AzureVMPowerManagement.'
        }
...
if (Test-Path -Path $scriptPath) {
    ...
    $index.Add('|---|---|')
    $index.Add("| [Invoke-AzureVMPowerManagementRunbook.ps1](Invoke-AzureVMPowerManagementRunbook.md) | $(Format-Cell (Format-HelpText (Get-Help -Name $scriptPath).Synopsis)) |")
    $index.Add('')
}
